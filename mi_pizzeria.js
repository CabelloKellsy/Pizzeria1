...
function seleccionarIngredientes(callback) {
    rl.question('\nIngrese los números de los ingredientes separados por comas (por ejemplo, 1,2,3 ): ', (respuesta) => {
        ...
        // Mostrar los ingredientes seleccionados para depuración
...
function guardarPedido(masaSeleccionada, ingredientesSeleccionados, precioTotal) {
    ...
    - Masa: ${masaSeleccionada.tipo} - Precio: ${calcularPrecioBase(masaSeleccionada.precio).toFixed(2)}€
    - Ingredientes:
    ${ingredientesSeleccionados.map(item => {
        const [nombre, precio] = Object.entries(item)[0];
        return `* ${nombre} - Precio: ${calcularPrecioBase(parseFloat(precio)).toFixed(2)}€`;
    }).join('\n')} \nPrecio total: ${precioTotal.toFixed(2)}€ `;

    //guardar en un archivo pedido.txt
    fs.writeFile('pedido.txt', contenidoPedido, (err) => {
        if (err) throw err;
